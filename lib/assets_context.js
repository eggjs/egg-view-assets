...
class Assets {
  ...
  get host() {
    if (this.isLocal) return `http://127.0.0.1:${this.config.devServer.port}`;
  ...
  getScript(entry) {
    ...
    if (common) {
      script += scriptTpl({ url: this.getURL('common.js') });
    }
  ...
  getURL(entry) {
    ...
    return `${this.host}${this.config.publicPath}/${urlpath}`;
  }
